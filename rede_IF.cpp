...
void runsim(double W, double g, double seed) {
	...
	int net_size = 80000;			/* Number of neurons of the network*/
	...
	double vth = -50.0;//30.0;//-50.0;				/* Threshold potential*/
	double tau = 10.0;				/* Time constant: IF model = 10; Izhi model = 1 */
	...
	//double a_FS=0.02,b_FS=0.2,c_FS=-65,d_FS=2, a_RS=0.02,b_RS=0.2,c_RS=-65,d_RS=2;
	...
	for(int i = 0; i < net_size; i++) {
		...
		if(i>=0 && i < round(0.268* net_size)) {
			...
			external_input[i] = updateNeuronInput(1600, 8, W, adjust_factor, h, tmax);
			n[i].layer = 1; //L23e
			...
			external_input[i] = updateNeuronInput(1500, 8, W, adjust_factor, h, tmax);
			...
			//external_input[i] = updateNeuronInput(2000, 8, W, 1, h, tmax);
			external_input[i] = updateNeuronInput(2100, 8, W, adjust_factor, h, tmax);
			n[i].layer = 3;	//L4e
			...
			//external_input[i] = updateNeuronInput(1850, 8, W, 1, h, tmax);
			external_input[i] = updateNeuronInput(1900, 8, W, adjust_factor, h, tmax);
			n[i].layer = 4;	//L4i
			...
			external_input[i] = updateNeuronInput(2000, 8, W, adjust_factor, h, tmax);
			//external_input[i] = updateNeuronInput(2000, 8, W, 1, h, tmax);
			n[i].layer = 5;	//L5e
			...
			external_input[i] = updateNeuronInput(2900, 8, W, adjust_factor, h, tmax);
			//external_input[i] = updateNeuronInput(2000, 8, W, 1, h, tmax);
			n[i].layer = 7;	//L6e
	...
	for(int i = 0; i < n_it; i++) {
		for(int j = 0; j < net_size; j++) {
			...
			if(n[j].acum_ref < t_ref ){
				...
				// LIF:
				n[j].v += (h/tau)*(-n[j].v + ext_input + n[j].syn_input[ind] + external_input[j][i]);
				
				// Izhikevich:
				//n[j].v += h*(0.04*n[j].v*n[j].v + 5*n[j].v + 140 - n[j].u + ext_input
				//  + n[j].syn_input[ind] + external_input[j][i]);				
